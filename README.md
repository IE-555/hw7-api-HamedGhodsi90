...
- The problem here is that we can retrieve just one page at the time by using the above code. So to take the first 100 pages, we build a loop in which we retrieve the data for one page, build jason file, then build a data frame using *pandas* package. Then it will repeat for the other pages, and at the end, we append all these data frames to have one single data frame which has 10,000 rows of data. The code of doing these steps is shown here:
...
# To build one comprehensive framework which contains all datasets we can retrieve from those 100 (total pages) pages. 
...
- The result shows that for example *Klay Thompson* from *Golden State Warriors* team could score the highest points in just one game **(52 points)**, or *Kyle lowry* from *Toronto Raptors* team had the most assist in one game **(15 assists)**.
...
![Image of Plot](hw7_histogram.jpg)

- This figure shows that from 2477 players which scored at least one point in a game, more than 1400 of them scored less than 10 points, while the top player (Klay Thompson) has the highest score with 52 points. It shows the difficulty of his work to score more than 50 points in a game which is very rare in NBA basketball.  
...
Since we retrieved a big data sets including 10,000 rows of data, we can do lots of analysis based on that. As some examples: 
